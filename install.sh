...
# This install all packages along with aur
  ...
  sudo pacman -Syu maim xclip xdotool picom libxcb arandr --no-confirm
  git clone https://aur.archlinux.org/yay.git
  cd yay
  makepkg -si --noconfirm
  cd ..
  yay -S udiskie aurutils nwg-look timeshift 
...
sudo pacman -Syu --needed base-devel nvim w3m github-cli neofetch firefox ranger zsh htop feh pamixer pavucontrol flatpak ueberzug atool poppler imagemagick highlight zathura-pdf-poppler brightnessctl dunst pacman-contrib acpi gvim mpv ncmpcpp bluez bluez-utils --no-confirm
git clone https://aur.archlinux.org/yay.git
cd yay
makepkg -si
cd ..
